...
from tensorflow.keras.layers import Dense, Conv3D, Reshape, MaxPooling3D, BatchNormalization, Input, Flatten, Dropout, \
    Activation, Add, Conv3DTranspose, Multiply, Lambda, Embedding, Concatenate
...
class ResNet_model_class():
    def __init__(self, N_b_a, N_b_rf, N_u_a, N_u_rf, N_s, K, PTRS_seperation, SNR, P, N_c, N_scatterers,
                 angular_spread_rad, wavelength, d, BATCHSIZE, phase_shift_stddiv,
                 truncation_ratio_keep, Nsymb, Ts, fc, c, mat_fname,
    ...
    def resnet_function_transceiver_small_sys(self, trainable_csi, trainable_ns, layer_name):
        ...
        ns = Input(shape=[self.Nsymb], batch_size=self.BATCHSIZE)

        # ns path
        FC_ns = Dense(units=self.K * self.N_b_a * self.N_u_a *2, activation='relu', use_bias=True)
        reshaper_ns = Reshape(target_shape=[self.K, self.N_b_a, self.N_u_a, 2])
        # common path
    ...
    def resnet_function_transceiver_large_sys(self, trainable_csi, trainable_ns, layer_name):
        kernels = [min(self.K, self.convolutional_kernels),
                   min(self.N_u_a, self.convolutional_kernels),
                   min(self.N_b_a, self.convolutional_kernels)]
        csi = Input(shape=(self.K, self.N_u_a, self.N_b_a, 2), batch_size=self.BATCHSIZE)
        ...
        # ns path
        FC_ns = Dense(units=round(self.K * self.N_b_a * self.N_u_a * 2 /
                                  (2*self.subcarrier_strides**2 *self.N_b_a_strides**2 *self.N_u_a_strides**2) ), activation='relu', use_bias=True)
        reshaper_ns = Reshape(target_shape=
                              [round(self.K/(2*self.subcarrier_strides**2)),
                               round(self.N_b_a/self.N_b_a_strides**2),
                               round(self.N_u_a/self.N_u_a_strides**2),
                               2])
        # common path
        MP_1 = MaxPooling3D(pool_size=(2 * self.subcarrier_strides, self.N_u_a_strides, self.N_b_a_strides))
        ID_block_common_branch_layer_1 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                kernel_size=kernels,
                                                                strides=self.convolutional_strides,
                                                                dilation=self.convolutional_dilation,
                                                                trainable=trainable_csi)
        MP_2 = MaxPooling3D(pool_size=(self.subcarrier_strides, self.N_u_a_strides, self.N_b_a_strides))
        ID_block_common_branch_layer_2 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                kernel_size=kernels,
                                                                strides=self.convolutional_strides,
                                                                dilation=self.convolutional_dilation,
        ...
        ID_block_common_branch_layer_4 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                ...
                                                                dilation=self.convolutional_dilation,
                                                                trainable=trainable_csi)
        ID_block_common_branch_layer_5 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                kernel_size=kernels,
                                                                strides=self.convolutional_strides,
                                                                dilation=self.convolutional_dilation,
                                                                trainable=trainable_csi)
        ID_block_common_branch_layer_6 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                kernel_size=kernels,
                                                                strides=self.convolutional_strides,
                                                                dilation=self.convolutional_dilation,
                                                                trainable=trainable_csi)
        ID_block_common_branch_layer_7 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                kernel_size=kernels,
                                                                strides=self.convolutional_strides,
                                                                dilation=self.convolutional_dilation,
                                                                trainable=trainable_csi)
        ID_block_common_branch_layer_8 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                kernel_size=kernels,
                                                                strides=self.convolutional_strides,
                                                                dilation=self.convolutional_dilation,
                                                                trainable=trainable_csi)
        ...
        # V_D path
        ID_block_D_branch_layer = []
        for i in range(3):
            ID_block_D_branch_layer.append(IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                kernel_size=kernels,
                                                                strides=self.convolutional_strides,
                                                                dilation=self.convolutional_dilation,
                                                                trainable=trainable_csi))

        Tconv_V_D_1 = Conv3DTranspose(filters=self.convolutional_filters,
                                    kernel_size=(self.subcarrier_strides, self.N_u_a_strides, self.N_b_a_strides),
                                    strides=(self.subcarrier_strides, self.N_u_a_strides, self.N_b_a_strides),
                                    padding='same')
        ID_block_D_branch_layer_end = IdentityBlock_v2_class(filters=2,
                                                           kernel_size=kernels,
                                                           strides=self.convolutional_strides,
                                                           dilation=self.convolutional_dilation,
                                                           trainable=trainable_csi)
        Tconv_V_D_2 = Conv3DTranspose(filters=2,
                                    kernel_size=(2*self.subcarrier_strides, self.N_u_a_strides, self.N_b_a_strides),
                                    strides=(2*self.subcarrier_strides, self.N_u_a_strides, self.N_b_a_strides),
                                    padding='same')
        MP_D_branch_end = MaxPooling3D(pool_size=(1, int(self.N_u_a / self.N_b_rf), int(self.N_b_a / self.N_s)))
        reshaper_D = Reshape(target_shape=[self.K, self.N_b_rf, self.N_s, 2])
        ...
        ID_block_RF_branch_layer_8 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            ...
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_9 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_10 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_11 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_12 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_13 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_14 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_15 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_16 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_17 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_18 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_19 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_20 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_21 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_22 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_23 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_24 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_25 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_26 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_27 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_28 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_29 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ID_block_RF_branch_layer_30 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        Tconv_V_RF_1 = Conv3DTranspose(filters=self.convolutional_filters,
                                    ...
                                    strides=(self.subcarrier_strides, self.N_u_a_strides, self.N_b_a_strides),
                                    padding='same')
        ID_block_RF_branch_layer_31 = IdentityBlock_v2_class(filters=1,
                                                            kernel_size=kernels,
                                                            strides=self.convolutional_strides,
                                                            dilation=self.convolutional_dilation,
                                                            trainable=trainable_csi)
        ...
        # connections
        x = MP_1(csi)
        x = ID_block_common_branch_layer_1(x)
        x = MP_2(x)
        x = ID_block_common_branch_layer_2(x)
        x = ID_block_common_branch_layer_3(x)
        x = ID_block_common_branch_layer_4(x)
        x = ID_block_common_branch_layer_5(x)
        x = ID_block_common_branch_layer_6(x)
        x = ID_block_common_branch_layer_7(x)
        x = ID_block_common_branch_layer_8(x)
        ...
        vd = x
        for i in range(3):
            vd = ID_block_D_branch_layer[i](vd)
        vd = Tconv_V_D_1(vd)
        vd = ID_block_D_branch_layer_end(vd)
        vd = Tconv_V_D_2(vd)
        vd = MP_D_branch_end(vd)
        vd = reshaper_D(vd)
        ...
        vrf = ID_block_RF_branch_layer_8(vrf)
        vrf = ID_block_RF_branch_layer_9(vrf)
        vrf = ID_block_RF_branch_layer_10(vrf)
        vrf = ID_block_RF_branch_layer_11(vrf)
        vrf = ID_block_RF_branch_layer_12(vrf)
        vrf = ID_block_RF_branch_layer_13(vrf)
        vrf = ID_block_RF_branch_layer_14(vrf)
        vrf = ID_block_RF_branch_layer_15(vrf)
        vrf = ID_block_RF_branch_layer_16(vrf)
        vrf = ID_block_RF_branch_layer_17(vrf)
        vrf = ID_block_RF_branch_layer_18(vrf)
        vrf = ID_block_RF_branch_layer_19(vrf)
        vrf = ID_block_RF_branch_layer_20(vrf)
        vrf = ID_block_RF_branch_layer_21(vrf)
        vrf = ID_block_RF_branch_layer_22(vrf)
        vrf = ID_block_RF_branch_layer_23(vrf)
        vrf = ID_block_RF_branch_layer_24(vrf)
        vrf = ID_block_RF_branch_layer_25(vrf)
        vrf = ID_block_RF_branch_layer_26(vrf)
        vrf = ID_block_RF_branch_layer_27(vrf)
        vrf = ID_block_RF_branch_layer_28(vrf)
        vrf = ID_block_RF_branch_layer_29(vrf)
        vrf = ID_block_RF_branch_layer_30(vrf)
        vrf = Tconv_V_RF_1(vrf)
        vrf = ID_block_RF_branch_layer_31(vrf)
