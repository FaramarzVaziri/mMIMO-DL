...
if __name__ == '__main__':
    ...
    dataset_name = 'Dataset_K4_Na4_rf2/DS1.mat'
    ...
    if create_DS_phase_noise_free == 'yes':
    ...
    the_model_tx = obj_neural_net_model.CNN_transceiver(trainable_ns=True, trainable_csi=True,
                                                        layer_name='TX')
    print('-- TX resnet model is created')
    the_model_rx = obj_neural_net_model.CNN_transceiver(trainable_ns=True, trainable_csi=True,
                                                        layer_name='RX')
    ...
    obj_capacity_metric = loss_phase_noised_class(N_b_a, N_b_rf, N_u_a, N_u_rf, N_s, K, SNR, P, N_c, N_scatterers,
                                                  angular_spread_rad, wavelength,
                                                  d, BATCHSIZE, truncation_ratio_keep, Nsymb,
                                                  sampling_ratio_time_domain_keep_capacity_metric,
                                                  sampling_ratio_subcarrier_domain_keep, 'eval', impl)
    ...
    obj_ML_model_pre_training = ML_model_class(the_model_tx, the_model_rx, N_b_a, N_b_rf, N_u_a, N_u_rf, N_s, K,
                                               ...
                                               sampling_ratio_time_domain_keep_capacity_metric,
                                               Nsymb, Ts, fc, c, PHN_innovation_std, dataset_name, eval_dataset_size,
                                               'train', False)
    ...
    if (load_trained_best_model == 'yes'):
        ...
        obj_ML_model_pre_training.load_weights('saved_model_weights/my_model_small.h5')
    ...
    if (do_pre_train == 'yes'):
        ...
        h_pre = obj_ML_model_pre_training.fit(the_dataset_train,
                                              ...
                                              verbose=1)
        if (save_pre == 'yes'):
            obj_ML_model_pre_training.save_weights('saved_model_weights/pre/my_model_small.h5')
            obj_ML_model_pre_training.save_weights('saved_model_weights/my_model_small.h5')
    ...
    if (do_post_train == 'yes'):
        ...
        h_post = obj_ML_model_post_training.fit(the_dataset_train_phn,
                                                ...
                                                verbose=1)  #
        if (save_post == 'yes'):
            obj_ML_model_post_training.save_weights('saved_model_weights/post/my_model_small.h5')
            obj_ML_model_post_training.save_weights('saved_model_weights/my_model_small.h5')
...
if (record_loss == 'yes'):
    try:
        mdic_losses = {"tr_loss": np.concatenate(
            [h_pre.history['neg_capacity_train_loss'], h_post.history['neg_capacity_train_loss']]),
                       'ts_loss': np.concatenate(
                           [h_pre.history['val_neg_capacity_test_loss'], h_post.history['val_neg_capacity_test_loss']]),
                       'metric': np.concatenate([h_pre.history['val_neg_capacity_performance_metric'],
                                                 h_post.history['val_neg_capacity_performance_metric']]),
                       'TL_start': epochs_pre}
    except:
...
if (evaluate_post == 'yes'):
    ...
    obj_sequential_loss_phase_noised_class_accurate = loss_phase_noised_class(N_b_a, N_b_rf, N_u_a, N_u_rf, N_s, K, SNR,
                                                                              P, N_c, N_scatterers, angular_spread_rad,
                                                                              wavelength,
                                                                              d, BATCHSIZE, truncation_ratio_keep,
                                                                              Nsymb,
                                                                              sampling_ratio_time_domain_keep_capacity_metric,
                                                                              sampling_ratio_subcarrier_domain_keep,
                                                                              'eval', impl)
...
try:
    train_loss = -h_post.history['neg_capacity_train_loss'][-1]
    test_capacity = -h_post.history['val_neg_capacity_performance_metric'][-1]
except:
    train_loss = -h_pre.history['neg_capacity_train_loss'][-1]
    test_capacity = -h_pre.history['val_neg_capacity_performance_metric'][-1]
...
if (record_loss == 'yes'):
    try:
        ...
        mdic_losses = {"tr_loss": h_pre.history['neg_capacity_train_loss'],
                       'ts_loss': h_pre.history['val_neg_capacity_test_loss'],
                       'metric': h_pre.history['val_neg_capacity_performance_metric'],
                       'TL_start': epochs_pre}
