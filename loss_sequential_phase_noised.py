...
class sequential_loss_phase_noised_class:
    ...
    def h_tilde_k_func(self, bundeled_inputs_0):
        [H_masked, Lambda_B_cyclshifted_masked, Lambda_U_cyclshifted_masked] = bundeled_inputs_0 # [k, ...]

        for k in range(int(self.K * self.truncation_ratio_keep)):
            T0 = tf.linalg.matmul(Lambda_U_cyclshifted_masked[k], H_k)
    ...
    def Rx_per_k(self, bundeled_inputs_0):
        ...
        for q in range(int(self.K * self.truncation_ratio_keep)):
            H_tilde_k = H_tilde_k + tf.linalg.matmul(
                tf.linalg.matmul(Lambda_U_cyclshifted_masked[q], H_masked[q]), Lambda_B_cyclshifted_masked[q])
    ...
    def R_I_Q_m_k(self, bundeled_inputs_0):
        ...
        if (m == k):
            ...
            for q in range(self.K * self.truncation_ratio_keep):
                H_hat_m_k = H_hat_m_k + tf.linalg.matmul(
                    tf.linalg.matmul(Lambda_U_cyclshifted_masked[q,:], H_masked[q,:], adjoint_a=False, adjoint_b=False),
    ...
    def Rq_per_k(self, bundeled_inputs_0):
        ...
        for m in range(self.K):
            RQ = RQ + tf.cast(self.R_N_Q_m_k([Lambda_U[tf.math.floormod(k - m, self.K), :], W_D[k,:], W_RF]), tf.complex64)+\
