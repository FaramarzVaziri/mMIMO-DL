...
from dataset_generator_from_file import dataset_generator_from_file_class
...
from dataset_generator_from_generator import dataset_generator_from_generator_class
...
if __name__ == '__main__':
    ...
    L_rate = 1e-6  # float(input("inital lr: "))
    ...
    obj_dataset_generator_from_generator_class = dataset_generator_from_generator_class(N_b_a, N_b_rf,
                                                                                        N_u_a, N_u_rf,
                                                                                        N_s, K, SNR,
                                                                                        P, N_c, N_scatterers,
                                                                                        angular_spread_rad,
                                                                                        wavelength, d, BATCHSIZE,
                                                                                        1,
                                                                                        Nsymb, Ts, fc, c,
                                                                                        PHN_innovation_std)

    my_dataset = obj_dataset_generator_from_generator_class.dataset_generator()
    # print(my_dataset)
    # my_dataset = my_dataset.cache()
    # AUTOTUNE = tf.data.experimental.AUTOTUNE
    # my_dataset = my_dataset.prefetch(BATCHSIZE)
    print(my_dataset)
    ...
    # B. ML model creation
    obj_CNN_model = CNN_model_class(N_b_a, N_b_rf, N_u_a, N_u_rf, N_s, K, P, BATCHSIZE, width_of_network, dropout_rate)
    ...
    obj_capacity_metric = paralle_loss_phase_noised_class(N_b_a, N_b_rf, N_u_a, N_u_rf, N_s, K, SNR, P, N_c, N_scatterers, angular_spread_rad, wavelength,
    ...
    obj_ML_model.fit(my_dataset, epochs=2000, validation_data=my_dataset, callbacks=[reduce_lr], validation_freq=10, verbose=1, workers = 8, use_multiprocessing= True)
    ...
    # obj_dataset_train_phn = dataset_generator_from_file_class(N_b_a, N_b_rf, N_u_a, N_u_rf, N_s, K, SNR, P, N_c, N_scatterers,
