...
class ResNet_model_class():
    ...
    def resnet_function_transceiver(self, trainable_csi, trainable_ns):
        ...
        ID_block_common_branch_layer_4 = IdentityBlock_v2_class(filters=self.convolutional_filters,
                                                                ...
                                                                dilation=self.convolutional_dilation,
                                                                trainable=trainable_csi)
        # mini-network of csi and ns
        csi_bn = BatchNormalization()
        csi_flatten = Flatten()
        csi_FC = Dense(units= 10, activation='relu', use_bias=True)
        FC_D = Dense(units= self.K* self.N_b_rf* self.N_s* 2, activation='relu', use_bias=True)
        add_D = Add()

        FC_RF = Dense(units= self.N_b_a, activation='relu', use_bias=True)
        reshaper_RF = Reshape(target_shape=[1, self.N_u_a, 1, 1])
        add_RF = Add()
        ...
        # self.K, self.N_u_a, self.N_b_a, 2
        # 1, 4, 1, 1
        # shape = [1, self.N_u_a, 1, 1]
        # connections
        ...
        y = csi_FC(y)
        y = csi_bn(y)
        vd_aux = FC_D(tf.concat([y, ns/self.Nsymb], axis=1))
        vd_aux = reshaper_D(vd_aux)
        vrf_aux = FC_RF(tf.concat([y, ns/self.Nsymb], axis=1))
        vrf_aux = reshaper_RF(vrf_aux)
        vd_end = add_D([vd, vd_aux])
        vrf_end = add_RF([vrf, vrf_aux])
